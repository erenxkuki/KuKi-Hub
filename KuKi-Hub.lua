...
local FolderName = "Ryzu hub | Farm"
...
-- =======================
-- TWEEN ADJUSTED BY DISTANCE
-- =======================

-- [[ VARIABLES FOR YOUR INPUT ]] --
getgenv().TweenSpeedFar = 300   -- Default Speed (Far)
getgenv().TweenSpeedNear = 900  -- Boost Speed (Close <= 90 studs)
...
-- ===============================  
--  IF WITHIN 90 STUDS → USE CLOSE SPEED
...
QuestCheck = function()
    ...
    -- [FIX] Keep level within quest limit
    if World1 and I > 699 then
        I = 650 -- Forces Galley Captain mission (Lv 650)
    end
    
    if World2 and I > 1499 then
        I = 1450 -- Forces Water Fighter mission (Lv 1450)
    end
...
elseif I == 1650 or I <= 1699 then
    ...
    PosM = CFrame.new(4674.92676, 1134.82654, 996.308838, 0.731321394, -0, -0.682033002, 0, 1, -0, 0.682033002, 0, 0.731321394)
		...
		elseif I >= 2600 and I <= 2624 then
			...
			-- Code to execute ONLY if already on the island
			Mon = "Reef Bandit"
			Qdata = 1
...
MaterialMon = function()
    ...
    SP = "Default";
    -- CORRECTED coordinate for Haunted Ship entrance (Sea 2)
...
local Window = bearlib:MakeWindow({
    Title = "Ryzu Hub | Farm",
    SubTitle = "by RealRyzu",
    SaveFolder = true,
    Image = "LOGO_IMAGE_ID" -- Replace with actual logo ID
})
...
v0:AddProfile({
    Name = "RealRyzu ☑",             
    Bio = "Ryzu Hub Developer", 
    Avatar = "AVATAR_IMAGE_ID", -- Replace with actual avatar ID
    Cover = "COVER_IMAGE_ID", -- Replace with actual cover ID
    Verified = true 
})
v0:AddSingleDiscordCard({
"Ryzu Hub 🇻🇳",
        "Follow our channels for updates!",
        "AVATAR_IMAGE_ID", -- Replace with actual image ID
        "BANNER_IMAGE_ID", -- Replace with actual banner ID
        2000,
        10000,
        "https://discord.gg/example" -- Replace with actual invite link
})

v0:AddParagraph({
    "Notice",
    "• Ryzu Hub\n• Script by RealRyzu\n• Creation Date: 7-6-2026\n• Last updated date: 7-6-2026\n• Gun aura currently cannot shoot boats and leviathan"
})

-- 1. Create Discord Invite Container with 2 cards
local discordContainer = v0:AddDiscordInvite({
    -- Card 1
    {
        "Join the TikTok Group",
        "Follow our TikTok channel for the latest updates!",
        "IMAGE_ID_1", -- Replace with actual image ID
        "BANNER_ID_1", -- Replace with actual banner ID
        500,
        1500,
        "https://tiktok.com/@example" -- Replace with actual link
    },
    -- Card 2
    {
        "Join the Discord group",
        "Join the Discord group to stay updated with the latest announcements.",
        "IMAGE_ID_2", -- Replace with actual image ID
        "BANNER_ID_2", -- Replace with actual banner ID
        250,
        1000,
        "https://discord.gg/example" -- Replace with actual invite link
    }
...
task.spawn(function()
    while true do
        ...
        task.wait(getgenv().GunAuraSettings.FireRate) -- FireRate is still 0.01
    end
end)
...
_G.AutoTyrant = false -- Added
_G.AcceptQuest = false

-- Variable to store Level Farm mob
local CurrentMobLevel = nil
local CurrentMobLevelSystem = nil

-- Threshold for conditional teleport (in studs)
local TELEPORT_DISTANCE_THRESHOLD = 15 

-- Helper function for conditional teleport
...
----------------------------------------------------------------------------
-- 1. UI: DROPDOWN + TOGGLES (Place in your UI section)
----------------------------------------------------------------------------
...
v2:AddToggle({
    ...
    Callback = function(v)
        ...
        -- Reset all control variables to avoid conflicts
        _G.Level = false  
        _G.AutoFarm_Bone = false  
        _G.AutoFarm_Cake = false  
        _G.AutoTyrant = false 
        ...
        if v then  
            ...
            elseif _G.SelectedFarmMode == "Tyrant Of The Skies" then  
                _G.AutoTyrant = true -- Activates Tyrant logic
            end  
        end
...
-- Settings
local FarmHeight = 45 -- Safe height to avoid getting stuck in mob

-- Enhanced NoClip (To not get stuck on walls/mobs)
spawn(function()
...
-- Check how many "Eyes" (Pots) have been broken
...
-- MAIN FARM LOOP
spawn(function()
    local VaseIndex = 1 -- Index to control which pot to attack
    
    while task.wait() do
        -- Only runs if toggle is on AND mode is Tyrant
        if _G.AutoTyrant and _G.StartFarm then
            pcall(function()
                local plr = game.Players.LocalPlayer
                ...
                -- [PRIORITY 2] BREAK 4 POTS TO SUMMON
                elseif eyes == 4 then
                    local targetsty = {
                    ...
                    -- FIX: Uses VaseIndex to cycle through pots instead of resetting loop
    ...
    while task.wait() do
        if _G.AutoFarm_Bone and _G.StartFarm then
            pcall(function()
                ...
                local npcPos = CFrame.new(-9516.99, 172.01, 6078.46) -- Bone NPC position

                -- [STEP 1] IF NEEDS QUEST, GO TO NPC FIRST
                if _G.AcceptQuest and not QuestUI.Visible then      
                    local distToNPC = (npcPos.Position - hrp.Position).Magnitude
                    
                    if distToNPC > 5 then
                        _tp(npcPos) -- Teleport directly to NPC
                    else
                        -- If close, try to take quest
                        task.wait(0.5)
                        local quests = {
                        ...
                        -- Try to take quest based on level (optional: kept random as in original)
                        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer(unpack(quests[math.random(1,#quests)]))
                        task.wait(1) -- Wait for quest to load in UI
                    end

                -- [STEP 2] IF ALREADY HAS QUEST OR DOESN'T WANT TO ACCEPT, GO FARM
                else
...
-- Increase to pick up mobs a bit further, decrease for very close.
...
v2:AddToggle({
    ...
    -- 1. Loads whether it was on or off when opening script
    Default = GetSetting("AutoFarmNear_Save", false),
    Callback = function(I)
        _G.AutoFarmNear = I
        
        -- 2. Stores in settings table for Save
        _G.SaveData["AutoFarmNear_Save"] = I
        
        -- 3. Saves to Settings.json file in executor folder
        SaveSettings()
    end,
})
...
spawn(function()
    while task.wait() do
        pcall(function()
            ...
            -- 🔍 1. Search for enemies in WORKSPACE
            ...
            -- 🔍 2. Search for enemies in REPLICATEDSTORAGE
...
-- System delay (adjustable)
local Sec = 0.5

if World2 then
v2:AddToggle({
    Name = "Auto Factory Raid",
    -- 1. Attempts to load whether it was on or off
    Default = GetSetting("AutoFactory_Save", false),
    Callback = function(Value)
        _G.AutoFactory = Value
        
        -- 2. Stores info in save table
...
v2:AddToggle({
    ...
    Callback = function(I)
        ...
        -- 3. Saves to Settings.json automatically
        SaveSettings()
    end,
})
...
-- Auto Collect Chest Button
v2:AddToggle({
    Name = "Auto Collect Chest",
    -- 1. Load saved state (or false by default)
    Default = GetSetting("AutoFarmChest_Save", false),
    Callback = function(I)
        _G.AutoFarmChest = I
        
        -- 2. Store in save table
        _G.SaveData["AutoFarmChest_Save"] = I
        
        -- 3. Save to Settings.json file
        SaveSettings()
    end,
})

-- Auto Collect Berry Button
v2:AddToggle({
...
v2:AddDropdown({
	...
	-- 1. Load saved material or leave empty if first time
	Default = GetSetting("SelectMaterial_Save", nil),
	Multi = false,
	Callback = function(I)
		(getgenv()).SelectMaterial = I
        
        -- 2. Save chosen material
        _G.SaveData["SelectMaterial_Save"] = I
        SaveSettings()
	end,
})

-- Auto Farm Toggle (Material)
v2:AddToggle({
	Name = "Auto Farm",
	-- 1. Load if material farm was on
...
-- Boss status check frame (from v1.txt)
...
-- Define mob list for each island
...
-- Variable storing island choice
_G.SelectedIsland = "Cake" -- Default is "Cake"

-- Function to get nearest mob from list
...
-- Skill Control for Fruit Mastery
...
-- Function to use Fruit skills based on settings
            ...
            pcall(function()
                ...
                if mob then
                    ...
                    repeat
                        ...
                        -- Use logic from Gravity Hub's G.Mas but fixed
            ...
            pcall(function()
                ...
                if mob then
                    ...
                    repeat
                        ...
                        -- Use logic from Gravity Hub's G.Masgun
                        if not mob:GetAttribute("Locked") then
                            mob:SetAttribute("Locked", mob.HumanoidRootPart.CFrame)
                        end
                        PosMon = (mob:GetAttribute("Locked")).Position
                        ...
                        -- Special gun handling (Skull Guitar)
                        ...
                        if tonumber(item.Mastery) >= 1 and tonumber(item.Mastery) <= 599 then
                            if GetBP(swordName) then
                                if mob then
                                    repeat
                                        wait()
                                        -- Use logic from Gravity Hub's G.Sword
                                        if not mob:GetAttribute("Locked") then
                                            mob:SetAttribute("Locked", mob.HumanoidRootPart.CFrame)
                                        end
                                        PosMon = (mob:GetAttribute("Locked")).Position
                            ...
                            break -- Only farm one sword at a time
                        elseif tonumber(item.Mastery) >= 600 then
                            -- Skip if mastery is maxed
                        end
                        break
                    end
                end
    ...
    while task.wait(0.5) do
        if _G.AutoFishing then
            pcall(function()
                ...
                -- Check if selected rod is equipped
                if _G.SelectedRod and (not equippedTool or equippedTool.Name ~= _G.SelectedRod) then
                    ...
                    if rodInBag then
                        ...
                        -- Use bearlib notification
                        bearlib:Notify({
                            Title = "Auto Fish",
                            Message = "please equip: " .. tostring(_G.SelectedRod),
                            Duration = 3
                        })
                        return -- Stop execution here to avoid errors trying to fish without rod
                    end
                end
                ...
                if equippedTool then
                    ...
                    elseif serverState == "Biting" then
                        ...
                        -- Use bearlib notification when fish caught
                        bearlib:Notify({
                            Title = "Auto Fish",
...
-- ========================================
-- AUTO QUEST SEA 2 (DRESSROSA) - FROM GRAVITY HUB
-- ========================================
if World1 then
    ...
    -- Auto Quest Sea 2 Logic
    spawn(function()
        while wait(Sec) do
            pcall(function()
                if _G.TravelDres then
                    -- Only run when level requirement met
                    if plr.Data.Level.Value >= 700 then
                        ...
                        if IceDoor then
                            -- Case door is closed (not opened yet)
                            if IceDoor.CanCollide == true and IceDoor.Transparency == 0 then
                                -- Talk to detective to get key
                            ...
                            -- Case door is open (need to fight boss)
                                ...
                                if Enemies:FindFirstChild("Ice Admiral") then
                                    for _, e in pairs(Enemies:GetChildren()) do
                                        if e.Name == "Ice Admiral" and G.Alive(e) then
                                            ...
                                            -- After defeating, request travel
...
-- ========================================
-- AUTO QUEST SEA 3 (ZOU) - FROM GRAVITY HUB
-- ========================================
if World2 then
    ...
    -- Auto Quest Sea 3 Logic (Shortened and optimized)
    spawn(function()
        while wait(Sec) do
            pcall(function()
                if _G.AutoZou then
                    ...
                    if plr.Data.Level.Value >= 1500 then
                        ...
                        -- === STAGE 1: DO "SWAN PIRATES" QUEST (BartiloProgress == 0) ===
                        if bartiloProgress == 0 then
                            local QuestUI = plr.PlayerGui.Main.Quest
                            -- If no quest, go to Bartilo to accept
                            if not QuestUI.Visible then
                                ...
                                -- If quest accepted, find and kill "Swan Pirate"
                        ...
                        -- === STAGE 2: KILL BOSS "JEREMY" (BartiloProgress == 1) ===
                        ...
                        -- === STAGE 3: COMPLETE BARTILO (BartiloProgress == 2) ===
                        ...
                        -- === STAGE 4: GET "FLAMINGO ACCESS" BY TALKING TO TREVOR ===
                        else
                            -- Check if FlamingoAccess is unlocked
                            ...
                            if unlockables and unlockables.FlamingoAccess == nil then
                                -- Talk to Trevor to unlock (requires fruit worth 1M+ in inventory)
...
-- Logic for Auto Farm Observation
spawn(function()
    while wait(.2) do
        pcall(function()
            if _G.obsFarm then
                ...
                -- Movement logic based on dodges left
...
if World3 then
    ...
    spawn(function()
        while wait(Sec) do
            pcall(function()
                EliteProgressPara:SetDesc("Elite Progress :  " .. replicated.Remotes.CommF_:InvokeServer("EliteHunter", "Progress"));
            end);
        end;
    end);
        ...
        pcall(function()
            while wait(Sec) do
                if _G.Auto_Rainbow_Haki then
                    -- Copy Rainbow Haki logic from Bear hub.txt (line 2058-2154) here if needed
                    -- This part is quite long, you can add later if desired
                end;
            end;
        end);
    end);
...
-- ========================================
-- AUTO SAW SWORD (WORLD 1) - from Gravity Hub
-- ========================================
if World1 then
    v3:AddToggle({
        Name = "Auto Saw Sword",
...
-- ========================================
-- AUTO SWAN COAT (WORLD 2) - from Gravity Hub
-- ========================================
if World2 then
    v3:AddToggle({
        Name = "Auto Swan Coat",
...
-- ========================================
-- AUTO MARINE COAT (WORLD 1) - from Gravity Hub
-- ========================================
if World1 then
    v3:AddToggle({
...
-- ========================================
-- AUTO WARDEN SWORD (WORLD 1) - from Gravity Hub
-- ========================================
if World1 then
    v3:AddToggle({
...
-- ========================================
-- AUTO CYBORG SWORD (WORLD 1) - from Gravity Hub
-- ========================================
if World1 then
    v3:AddToggle({
...
-- ========================================
-- AUTO MIDNIGHT BLADE / CURSED CAPTAIN (WORLD 2) - from Gravity Hub
-- ========================================
if World2 then
    v3:AddToggle({
...
-- ========================================
-- AUTO RENGOKU (WORLD 2) - from Gravity Hub
-- ========================================
if World2 then
    v3:AddToggle({
...
-- ========================================
-- AUTO POLE V2 (WORLD 2 & 3) - from Gravity Hub
-- ========================================
if World2 or World3 then
    v3:AddToggle({
...
-- ========================================
-- AUTO BISENTO V2 (WORLD 1) - from Gravity Hub
-- ========================================
if World1 then
    v3:AddToggle({
...
-- ========================================
-- SKULL GUITAR (WORLD 3) - from Gravity Hub (Shortened version)
-- ========================================
if World3 then
    v3:AddToggle({
...
-- ========================================
-- CURSED DUAL KATANA (WORLD 3) - from Gravity Hub
-- ========================================
if World3 then
...
-- ========================================
-- TRUE TRIPLE KATANA (WORLD 3) - from Gravity Hub
-- ========================================
if World3 then
...
-- ========================================
-- AUTO SWAN GLASSES / DON SWAN (WORLD 2) - from Gravity Hub
-- ========================================
if World2 then
    v3:AddToggle({
...
-- ========================================
-- AUTO CANVENDISH SWORD (WORLD 3) - from Gravity Hub
-- ========================================
if World3 then
    v3:AddToggle({
...
-- ========================================
-- AUTO BIGMOM / CAKE QUEEN (WORLD 3) - from Gravity Hub
-- ========================================
if World3 then
    v3:AddToggle({
...
-- ========================================
-- AUTO KILL TRAINING DUMMY (WORLD 3) - from Gravity Hub
-- ========================================
if World3 then
    v3:AddToggle({
...
-- Check Prehistoric Island status
...
-- Function to use Prehistoric skills
...
-- ========================================
-- MAIN PREHISTORIC TOGGLES (From Gravity Hub)
-- ========================================
...
-- Auto Find Prehistoric Island Logic (from Gravity Hub)
...
-- Auto Start Prehistoric Event Logic
spawn(function()
    while wait() do
    ...
    while wait(0.3) do
        if _G.Prehis_Skills then
            pcall(function()
                ...
                -- Remove TouchInterest (except TrialTeleport)
...
-- Auto Collect Dragon Eggs Logic
spawn(function()
    while wait(Sec) do
...
-- ========================================
-- VOLCANIC CRAFTING (From Gravity Hub)
-- ========================================
...
-- ========================================
-- LOGIC FOR DRAGO TRIALS (from Gravity Hub)
-- ========================================
...
-- ========================================
-- DRAGON HUNTER (Blaze EM) - From Gravity Hub
-- ========================================
...
-- ========================================
-- UPGRADE RACES LOGIC (from Gravity Hub)
-- ========================================
...
-- Format number function (add commas)
    ...
    while wait(1) do
        pcall(function()
            if (getgenv()).AutoBuyFruitSniper and (getgenv()).SelectFruit then
                -- Call GetFruits to refresh stock
...
-- ========================================
-- FRUIT STOCK CHECKER (Enhanced version)
-- ========================================
v9:AddSection({"Fruit Stock Checker"})

-- Function to check specific fruit
...
-- Dropdown to select fruit to check
v9:AddDropdown({
...
-- Paragraph to display check result
...
-- Check stock every 30 seconds and notify if available
task.spawn(function()
    while task.wait(30) do
...
-- ADVANCED FRUIT DEALER ESP: NPC name \n M
...
-- HAKI COLOR DEALER ESP: NPC name \n M
...
-- Melee list with coordinates per World
local MeleeCoords = {
    ["Dark Step"] = {
        Key = "BuyBlackLeg",
...
-- Function to get available Melee in current Sea
        ...
        task.spawn(function()
            while _G.AutoBuyMelee do
                ...
                if not data or not data.Pos then
                    bearlib:Notify({
                        Title = "Ryzu Hub",
                        Message = "Error: Coordinates not found for this Melee in current Sea!",
                        Duration = 3
                    })
                ...
                if lp.Character and lp.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if dist > 15 then
                        -- Use _tp function from text file
                        _tp(data.Pos)
                    else
...
-- ========================================
-- BUY CRAFT ITEMS SEA EVENT
-- ========================================
v14:AddSection({"Buy Craft Items Sea Event"})

v14:AddButton({
    Name = "Craft Dragonheart",
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CraftItem", "Craft", "Dragonheart")
        bearlib:Notify({Title = "Craft", Message = "Crafted Dragonheart", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Craft", Message = "Crafted Dragonstorm", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Craft", Message = "Crafted DinoHood", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Craft", Message = "Crafted SharkTooth", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Craft", Message = "Crafted TerrorJaw", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Craft", Message = "Crafted SharkAnchor", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Craft", Message = "Crafted LeviathanCrown", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Craft", Message = "Crafted LeviathanShield", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Craft", Message = "Crafted LeviathanBoat", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Craft", Message = "Crafted LegendaryScroll", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Craft", Message = "Crafted MythicalScroll", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Geppo", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Buso Haki", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Soru", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Observation Haki", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Cutlass", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Katana", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Iron Mace", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Dual Katana", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Triple Katana", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Pipe", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Dual-Headed Blade", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Bisento", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Soul Cane", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Pole V2", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Slingshot", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Musket", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Flintlock", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("BuyItem", "Refined Flintlock") -- Fixed name
        bearlib:Notify({Title = "Shop", Message = "Purchased Refined Slingshot", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Refined Flintlock", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Cannon", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Kabucha", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Bizarre Rifle", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Black Cape", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Swordsman Hat", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Shop", Message = "Purchased Tomoe Ring", Duration = 2})
    end
})

-- ========================================
-- CHANGE RACE, RESET STATS
-- ========================================
v14:AddSection({"Change Race, Reset Stats"})

v14:AddButton({
    Name = "Change to Ghoul",
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Ectoplasm", "Change", 4)
        bearlib:Notify({Title = "Race", Message = "Changed to Ghoul", Duration = 2})
    end
})

v14:AddButton({
    Name = "Change to Cyborg",
    Callback = function()
        game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("CyborgTrainer", "Buy")
        bearlib:Notify({Title = "Race", Message = "Changed to Cyborg", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Stats", Message = "Stats Reset", Duration = 2})
    end
})
...
v14:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Race", Message = "Race Randomized", Duration = 2})
    end
})
...
v15:AddButton({
    ...
    Callback = function()
        ...
        if not RedeemRemote then
            bearlib:Notify({Title = "Error", Message = "Could not find Redeem remote", Duration = 3})
            return
        end
        ...
        bearlib:Notify({Title = "Success", Message = "Attempted to redeem all codes", Duration = 3})
    end
})
...
v15:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Graphics", Message = "Switched to Fast Mode", Duration = 2})
    end
})
...
v15:AddButton({
    ...
    Callback = function()
        LowCpu()
        bearlib:Notify({Title = "Optimization", Message = "Low CPU mode enabled", Duration = 2})
    end
})
...
v15:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Boats", Message = "Boat speed increased (client-side)", Duration = 2})
    end
})
...
v15:AddButton({
    ...
    Callback = function()
        ...
        bearlib:Notify({Title = "Fog", Message = "Fog removed", Duration = 2})
    end
})
...
-- VALUES FROM GRAVITY HUB
-- Gravity Hub uses:
--   (getgenv()).WalkSpeedValue = 30 (default)
--   (getgenv()).JumpValue = 50 (default)
--   _G.WalkSpeedEnabled to toggle
--   _G.JumpEnabled to toggle

-- Speed control variables (synced with Gravity Hub)
...
-- Function to apply stats (copied from Gravity Hub)
...
-- Function to monitor WalkSpeed changes (like Gravity Hub)
...
-- Monitor character changes (like Gravity Hub)
local plr = game.Players.LocalPlayer
...
-- Monitoring loop (kept from text file 1.txt)
...
-- Speed value input (synced with Gravity Hub)
v12:AddTextBox({
...
-- Jump value input (synced with Gravity Hub)
v12:AddTextBox({
...
-- ========================================
-- SECTION: LOCALPLAYER SETTINGS / MISC
-- ========================================
v12:AddSection({"LocalPlayer Settings / Misc"})

-- Infinite Mink V3 toggle
v12:AddToggle({
...
-- Infinite Observation Range toggle
v12:AddToggle({
...
-- ========================================
-- SECTION: COMBAT SETTINGS / AIMBOT SETTINGS
-- ========================================
v12:AddSection({"Settings Combat / Aimbot Settings"})

-- Ignore same teams toggle
v12:AddToggle({
